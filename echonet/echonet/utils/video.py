...
@click.option("--extract_frames", default=False)
...
def run(
    ...
    Args:
        ...
        extract_frames (bool, optional): Whether to extract two random frames in each epoch to train and test
            Defaults to False.
    ...
    # Set up model
    # pretrained models cannot be loaded on leomed, therefore they are pre-loaded in pretrainedModels/hub/checkpoints/
    if pretrained == False:
        try: # some models can be pre-loaded even if pretrained=False
            model_filename = "../pretrainedModels/hub/checkpoints/" + model_name + ".pt"
        ...
        model_filename = "../pretrainedModels/hub/checkpoints/" + model_name + "_pretrained.pt"
        model = torch.load(model_filename)
    ...
    with open(os.path.join(output, "log.csv"), "a") as f:
        ...
        for epoch in range(epoch_resume, num_epochs+1):
            ...
            for phase in ['train', 'val']:
                ...
                dataloader = torch.utils.data.DataLoader(
                    ds, batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=(device.type == "cuda"), 
        ...
        if run_test:
            for split in ["val", "test"]:
                ...
                if split == "test":
                    runtimes["test"] = time.time() - test_start_time
                ...
                for thresh in [35, 40, 45, 50]:
                    ...
                    print(thresh, sklearn.metrics.average_precision_score(y > thresh, yhat))
                    plt.plot(fpr, tpr)

                    if thresh == 50:
                        auroc = sklearn.metrics.roc_auc_score(y > thresh, yhat)
                        auprc = sklearn.metrics.average_precision_score(y > thresh, yhat)
                        r_squared = sklearn.metrics.r2_score(y, yhat)
